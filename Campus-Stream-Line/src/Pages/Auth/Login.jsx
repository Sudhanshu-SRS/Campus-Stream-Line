...
const Login = ({ role = "institute" }) => {
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center relative overflow-hidden px-4 ${theme.pageBg}`}
    >
      ...
      >
        ...
        <div className="flex justify-center mb-4">
          <span
            className={`px-5 py-2 rounded-full text-sm font-semibold flex items-center gap-2 shadow ${theme.badge}`}
          >
            <Sparkles size={16} />
            {role === "student" ? "Student Login" : "Institute Login"}
          </span>
        ...
        <div className="text-center mb-8">
          <h1
            className={`text-3xl font-extrabold ${theme.text} animate-textGlow`}
          >
            {theme.title}
          </h1>
          <p className={`mt-2 ${theme.muted}`}>{theme.subtitle}</p>
